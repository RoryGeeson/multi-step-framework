...
class NeuralProcess(tf.keras.Model):
    ...
    def __init__(self, x_dim, y_dim, layer_dim, r_dim, z_dim, num_samples, optimizer, learning_rate):
        ...
        self.optimizer = self.optimizer_dict[optimizer](learning_rate)
    ...
    def visualise(self, x, y, x_star, plot_var, stage):
        ...
        for j in range(mu.shape[2]):
            ...
            plt.savefig('C:/Users/geeso/OneDrive - University of Cambridge/Year_1/Pietro/multi_step/examples/Toy_problem/Plots/Stage_Plots/Stage_{}_y_{}.png'.format(stage,j),
                        bbox_inches='tight')
            plt.clf()
            plt.cla()
            plt.close()
