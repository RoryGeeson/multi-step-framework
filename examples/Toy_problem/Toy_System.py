...
# import neural_processes as neurpro
...
class Toy():
    ...
    def __init__(self):
        ...
        self.solventDict = {1:[0.347,0.7],2:[0.5,0.6],3:[0.7,0.4]}

        self.solventCostDict = {1: 1, 2: 1.5, 3: 2}
        ...
        self.conditionTypes = ['split', 'Temp', 'Pressure', 'solvent', 'time', 'Temp', 'Pressure', 'time', 'Temp', 'Pressure', 'time']

    def stage_1_ODEs(self, species, t, T1, P1, solvent):
        """
        ...
        A_B = self.solventDict[round(solvent)][0] * 500
        A_C = self.solventDict[round(solvent)][1] * 1000
    ...
    def stage_0(self, A, split_discrete):
        A1 = A * self.splitDict[round(split_discrete)]
        A2 = A - A1
        return A1,A2

    def stage_1(self, A1, T1, P1, solvent, t_out):
        t_out = np.linspace(0, t_out, 500)
        species = [1, 0, 0] # x_A1, x_B, x_C
        species_out = odeint(self.stage_1_ODEs, species, t_out, (T1, P1, solvent))
    ...
    def noiselessExperimentalRun(self, A, conditions):
        A1, A2 = self.stage_0(A, conditions[0])
        A1, B, C = self.stage_1(A1, *conditions[1:5])
        A2, B, D, E = self.stage_2(A2, B, *conditions[5:8])
        C, D, E, F = self.stage_3(C, D, E, *conditions[8:])
        return [A1, A2, B, C, D, E, F]
    ...
    # ['split', 'Temp', 'Pressure', 'solvent', 'time', 'Temp', 'Pressure', 'time', 'Temp', 'Pressure', 'time']
    ...
    #     # x_dim, y_dim, encoder_dim, r_dim, z_dim, optimizer, learning_rate
    #     model_stage_1 = NeuralProcess()
    #     for epoch in range(epochs):
    #         # data, target_fraction=0.2
    #         # [x, y]; x: function input (n_points,len_x), y: function output (n_points, len_y)
    #         model_stage_1.epoch(data)

    # def fit_stage_2(self,epochs=1000):
    #
    #
    # def fit_stage_3(self,epochs=1000):
